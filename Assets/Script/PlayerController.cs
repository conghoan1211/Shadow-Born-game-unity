...
namespace Assets.Script
{
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField] public int DragonBallCount = 7;
        [Space(5)]
        ...
        private void Hit(Transform _attackTransform, Vector2 _attackArea, ref bool _recoilDir, float _recoilStrength)
        {
            ...
            for (int i = 0; i < objectsHit.Length; i++)
            {
                ...
                {
                    objectsHit[i].GetComponent<Enemy>().EnemyHit(
                    damage, (transform.position - objectsHit[i].transform.position).normalized, _recoilStrength);

                    if (objectsHit[i].CompareTag("Enemy"))
                    {
                        Mana += manaGain;
                        if (hitSounds.Length > 0) // Kiểm tra mảng âm thanh không rỗng
                        {
                            int randomIndex = Random.Range(0, hitSounds.Length); // Chọn ngẫu nhiên một chỉ số trong mảng
                            hitAudioSource.clip = hitSounds[randomIndex]; // Gán âm thanh ngẫu nhiên vào AudioSource
                            hitAudioSource.PlayOneShot(hitAudioSource.clip); // Phát âm thanh
                        }
        ...
        {
            if (_other.GetComponent<Enemy>() != null && pState.shoot)
            {
                _other.GetComponent<Enemy>().EnemyHit(shootDamage, (_other.transform.position - transform.position).normalized, -recoilYSpeed);
            }
            if (_other.CompareTag("Finish"))
            {
                int dragonballNumber = Inventory.instance.GetItemQuantity("Dragonball");

                if (dragonballNumber < DragonBallCount)
                {
                    GameManagerGO.GetComponent<_GameController>().IsShowMissionPanel(true);
                    return;
                }
                else
                {
                    GameManagerGO.GetComponent<_GameController>().SetGameManagerState(_GameController.GameManagerState.Victory);
                    walkSpeed = 0;
                    anim.SetTrigger("Win");
                    pState.shoot = false;
                    pState.jumping = false;
        ...
        void Die()
        {
            ...
            if (currentHealth <= 0 && !hasDied)
            {
                ...
                GameManagerGO.GetComponent<_GameController>().SetGameManagerState(_GameController.GameManagerState.GameOver);
            }
